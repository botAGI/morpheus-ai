...
def test_generate_training_script_rejects_symlinked_ancestor_directory(tmp_path):
    external_dir = tmp_path / "external-scripts"
    external_dir.mkdir()
    scripts_dir = tmp_path / "scripts"
    try:
    ...
    with pytest.raises(ValueError, match="Training script directory must not contain a symlink"):
        generate_training_script({}, scripts_dir / "nested" / "morpheus_train.sh")

    assert not (external_dir / "nested" / "morpheus_train.sh").exists()
