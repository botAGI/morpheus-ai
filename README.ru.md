...
> Статус: beta release. Current beta package: 0.2.0b2. GitHub pre-release:
> [v0.2.0b2](https://github.com/botAGI/morpheus-ai/releases/tag/v0.2.0b2).
...
> Для v0.2-функций пиньте `morpheus-wake==0.2.0b2`; unpinned PyPI tools могут
...
- **v0.6 (завершён в текущем коде)**: audited
  ...
  через единую границу канонического пересчёта. Receipts от review apply
...
activation. Нет rollback - нет production activation.
...
Для `--from active-state` нужен versioned review authority внутри подписанного
receipt. Dataset проектирует только точные candidate-to-claim/evidence bindings
из `morpheus review apply`; unbound compiler claims исключаются. Обычный
compile/wake receipt остаётся chain-verifiable, но считается integrity-only и
не может разрешить active-state learning.
