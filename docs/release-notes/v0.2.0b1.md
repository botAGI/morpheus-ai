...
uvx --from 'morpheus-wake==0.2.0b1' morpheus check --input tests/fixtures/check_correct_input.txt --local
uvx --from 'morpheus-wake==0.2.0b1' morpheus learn lab . --dogfood --no-train
```

Pin the exact beta version for v0.2 features. Unpinned PyPI installers may
select the latest stable v0.1.1 instead of this beta.
